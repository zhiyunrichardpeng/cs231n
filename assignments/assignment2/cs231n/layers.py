...
# Add 
    ...
    H_prime = 1 + (H + 2 * P - HH) // S  # slide 94 lecture 5.
...
# Add 
        ...
        for f in range(F):                # each filter
            for i in range(H_prime):      # each output row
                for j in range(W_prime):  # each output column
                    ...
                    dpatch = w[f] * dout[n, f, i, j]  # dpatch is only a intermediate variable, so we dont accumulate that, like we did for dw[f]
    ...
    if P > 0:
        dx = dx_padded[:, :, P:-P, P:-P]  # eg: dx_padded[1:-1], means from 1, to the second last one.
    else:
        dx = dx_padded
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    # correcion:
    # x_reshape = x.np.reshape(N,H,W,C)
    x_transposed = x.transpose(0, 2, 3, 1)  # Shape goes from (N, C, H, W) -> (N, H, W, C)
    x_reshaped = x_transposed.reshape(N * H * W, C)  # Shape is now (N * H * W, C)


    out_reshaped, cache = batchnorm_forward(x_reshaped, gamma, beta, bn_param)
    # out = out_reshaped.np.reshape(N, C, H, W)
    out_transposed  = out_reshaped.reshape(N, H, W, C)
    out = out_transposed.transpose(0, 3, 1, 2)  # Shape goes from (N, H, W, C) -> (N, C, H, W)

    # for n in range(N):                    # each image in the batch
    #     for c in range(C):                # each filter
    ...
    #                 out[n, c, :, :], cache[n, c, :, :] = batchnorm_forward(patch, gamma, beta, bn_param)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    # x, x_hat, gamma, sample_mean, sample_var, eps = cache
    # correction:
    dout_transposed = dout.transpose(0, 2, 3, 1)  # Shape goes from (N, C, H, W) -> (N, H, W, C)
    dout_reshaped = dout_transposed.reshape(N * H * W, C)  # Shape is now (N * H * W, C)
    ...
    #                 dx[n, c, :, :], dgamma[n, c, :, :], dbeta[n, c, :, :] = batchnorm_backward(patch, cache)
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    # D, N = shape.x_t
    # for g in range(0,G):
    #     sample_mean = np.mean(x_t[g:g+D/G], axis=0)                      # (N,)
    #     sample_var = np.var(x_t[g:g+D/G], axis=0)                         # (N,)
    #     x_hat_t[g:g+D/G] = (x_t[g:g+D/G] - sample_mean) / np.sqrt(sample_var + eps)  # (D, N)
    ...
    mean = np.mean(x_reshaped, axis=(2, 3, 4), keepdims=True)  # Shape: (N, G, 1, 1, 1)
    var = np.var(x_reshaped, axis=(2, 3, 4), keepdims=True)    # Shape: (N, G, 1, 1, 1)
    ...
    cache = (x_reshaped, x_hat_reshaped, gamma, mean, var, eps, G)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    x_reshaped, x_hat_reshaped, gamma, mean, var, eps, G = cache
    # N, C, H, W = x_reshaped.shape
    # x_t: (D, N), x_hat_t: (D, N), gamma: (D,)
    ...
    #     # Step 1: backprop through out = gamma * x_hat + beta  (in N,D space)
    #     x_hat[g:g+D/G] = x_hat_t[g:g+D/G].T                                      # (N, D)


    #     dgamma[g:g+D/G] = np.sum(dout * x_hat[g:g+D/G], axis=0)                   # (D,)
    #     dbeta[g:g+D/G] = np.sum(dout, axis=0)                             # (D,)
    #     # ...
    #     # dx_hat = dout * gamma, then transpose to (D, N) for normalization backward
    ...
    '''
    dout has shape (N, C, H, W).
    The variable you unpacked from cache is x_hat_reshaped, which has shape (N, G, C // G, H, W).
    '''
    ...
    M = (C // G) * H * W
    sigma_eps = 1.0 / np.sqrt(var + eps)

    dx_reshaped = (1.0 / M) * sigma_eps * (
        M * dx_hat_reshaped
        - np.sum(dx_hat_reshaped, axis=(2, 3, 4), keepdims=True)
        - x_hat_reshaped * np.sum(dx_hat_reshaped * x_hat_reshaped, axis=(2, 3, 4), keepdims=True)
    )
