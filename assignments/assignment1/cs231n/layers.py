...
#     # 1. Numerical stability: shift scores
#     # x: [N x C], shift: [N x 1]
#     shifted_x = x - np.max(x, axis=1, keepdims=True)

#     # 2. Compute probabilities (for dx)
#     # exp_scores: [N x C]
#     exp_scores = np.exp(shifted_x)
#     # probs: [N x C]
#     probs = exp_scores / np.sum(exp_scores, axis=1, keepdims=True)

#     # 3. Compute Loss using the stable identity
#     # Loss = -log(probs[y]) = -[shifted_x[y] - log(sum(exp_scores))]
#     log_sum_exp = np.log(np.sum(exp_scores, axis=1))
#     correct_class_scores = shifted_x[np.arange(num_train), y]
    
#     # loss: scalar
#     loss = np.sum(log_sum_exp - correct_class_scores) / num_train

#     # 4. Compute Gradient dx (dL/dz)
#     # dx = (p - 1) for correct class, (p) for others
#     # dx: [N x C]
#     dx = probs.copy()
#     dx[np.arange(num_train), y] -= 1
    
#     # 5. Average the gradient
#     dx /= num_train
